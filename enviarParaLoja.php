...
<head>
    ...
    <link rel="stylesheet" href="css/enviarParaLoja.css">
...
<div id="container">
	...
	<div id="area-adicionarProdutos">
		<section class="container-background-adicionar" >
			<div class="container-form">
				...
				<!-- formulario de envio -->
				<form method="post" action="controller.php?acao=adicionar&pag=enviarParaLoja" id="formulario-produtos" >
					<fieldset>
    ...
    <section class="area-enviar-para-loja">
		<?php foreach ($recuperado as $indice => $recuperado){ ?>
			<div class="area-produtos-para-enviar">
				<div class="nome-produto-enviar">
					<h4><?= $recuperado->produto ?></h4>
				</div>

				<div class="buttons-enviarSN">
					<div >
						<?php if($recuperado->loja_estoque == 'estoque'){ ?>
							<button onclick="enviarEstoqueLoja('estoque', <?= $recuperado->id ?>)">Não enviar</button>
							<button style="background: rgb(0, 81, 255); color: white;">Enviar loja</button>
						<?php } ?>

						<?php  if($recuperado->loja_estoque == 'loja'){ ?>
							<button style="background: rgb(0, 81, 255); color: white;">Não enviar</button>
							<button onclick="enviarEstoqueLoja('loja', <?= $recuperado->id ?>)">Enviar loja</button>
						<?php } ?>
					</div>
				</div>
			</div>
